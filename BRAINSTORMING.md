...
- ~~Migrate engage-orchestrator.test.mjs from subprocess to DI-based tests~~ (added ~s1755, completed s1762 via wq-834)
- **Extract createCircuitStore test helper into shared test-utils** (added ~s1762): The in-memory circuit store factory created for engage-orchestrator.test.mjs (wq-834) duplicates the recordOutcome logic from lib/circuit-breaker.mjs. Extract it into a shared test-utils/circuit-store-mock.mjs so that future tests (e.g. filterByCircuit tests, new CLI handler tests) can reuse the same mock without duplicating the state management logic.
...
- **Fix conversation-balance.mjs TypeError** (added ~s1759): conversation-balance.mjs throws `TypeError: threads.map is not a function` at line 59 during E prehook execution. The `threads` variable from thread-tracker data isn't always an array. Needs a guard or format check. Currently non-blocking (check 4 continues on error) but generates noisy stderr in hook output.
## Active Observations
