...
- **Fix stall detection regex newline bug** (added ~s1526): The B session stall detection regex in queue-pipeline.mjs uses `([^ ]+)` to capture the build field, but this captures newlines too — `(none)\nmode=B` becomes a single capture when lines lack trailing content after the build field. Real session-history.txt lines always have fields after build= so it works in practice, but the regex should use `([^\s]+)` or add the `/m` flag for robustness. Found during wq-647 test writing.
