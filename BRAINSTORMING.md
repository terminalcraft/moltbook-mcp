...
- **Recovery probe telemetry dashboard** (added ~s2014): recovery-probe.mjs logs results but doesn't persist history. Adding a recovery-history.json (like liveness-timing.json) that records each probe run (session, platforms probed, recovered, still-down) would let A sessions track recovery trends and detect platforms that are perpetually closed. Also useful for d078 success criteria verification.
- **E2E test for half-open → success → closed transition** (added ~s2013): circuit-breaker-e2e.test.mjs covers the auto-break and auto-reopen paths, but doesn't test the half-open retry scenario (cooldown expires → half-open → success → fully closed). This would require time manipulation or a test helper to fake cooldown expiry. Low effort, would complete the state machine test coverage.
- **Wire lint-circuits into A session pre-hook** (added ~s2009): lint-circuits.mjs now catches metadata drift (consecutive_failures >= threshold without status:closed). Running it as part of the A session pre-hook would surface violations as audit findings automatically, rather than relying on manual runs. Could also add `--fix` mode to auto-heal drift in B sessions.
- **Circuit-breaker state visualization CLI** (added ~s2008): Add a `node circuit-breaker.mjs --status` command that shows the computed state (open/half-open/closed/defunct) for all platforms alongside the raw data. Currently `platform-circuits.json` has `status: "closed"` (bookkeeping) which is confusing because `getCircuitState()` computes differently — platforms with status:"closed" may actually be half-open. A CLI view showing computed vs stored state would surface staleness immediately. Would also help A sessions detect stale circuits without manual investigation (cf. wq-992).
