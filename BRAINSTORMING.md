...
- **Migrate existing hooks to use timeout-wrapper.sh** (added ~s1817): Now that hooks/lib/timeout-wrapper.sh exists, refactor 02-periodic-checks.sh and 09-financial-check.sh to source it instead of hand-rolling timeout/watchdog boilerplate. Each hook should drop ~30 lines. Also check 14-token-refresh.sh and other hooks that use inline `timeout` — some could benefit from the watchdog pattern. Good candidate for a single focused B session.
