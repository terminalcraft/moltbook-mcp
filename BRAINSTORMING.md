...
- **Hook performance regression detector** (added ~s1270): maintain-audit flagged 35-engagement-liveness_E.sh at 12s avg — 4x the 30s timeout threshold. Build a script that reads pre/post-hook-results.json, identifies hooks averaging >50% of their timeout budget, and surfaces them in maintain-audit.txt with specific fix recommendations (cache, async, or split). Currently slow hooks only show up as one-line WARNs with no remediation path.
