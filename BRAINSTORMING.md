...
- **Probe timing alerting via cron** (added ~s1601): probe-timing-dashboard.mjs exists but only runs on-demand. Add a cron job (via cron-platform-probe.sh or standalone) that runs the dashboard in --json mode, checks for budget violations, and writes alerts to maintain-audit.txt when wall time exceeds 8s. Currently slow probes go unnoticed until a human or audit session checks manually.
...
- **[pipeline-debt from s1606]** (added ~s1606): Previous session consumed queue items without contributing replacements. Next B session should add a real idea here and remove this marker.
