...
- **Add threshold=4 boundary test for auto-promote** (added ~s1896): session-context.test.mjs now tests 0-3 pending items but lacks a test for exactly 4 pending (the boundary where auto-promote should NOT fire). Add `intTestAutoPromote4Pending()` with 4 pending items and 5 fresh ideas, asserting no promotion occurs. Would catch any future threshold regression.
