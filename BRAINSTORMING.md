...
- ~~Apply same runner pattern to B/E/R prehooks~~ (promoted: E prehook → wq-983, R#357. B/R prehooks have fewer node calls — profile before creating wq items.)
- **imanagent-verify.mjs: add circuit-breaker awareness to token-refresh** (added ~s1985): 14-token-refresh.sh failing 100% (5/5 sessions) because imanagent.dev returns empty JSON. The Colony side has circuit-breaker awareness (R#354) but the imanagent side doesn't — and `set -euo pipefail` means the imanagent failure kills the entire hook. Fix: either add circuit-breaker check for imanagent before attempting refresh, or make the imanagent block non-fatal (trap errors). The hook should succeed when Colony refresh succeeds even if imanagent fails.
