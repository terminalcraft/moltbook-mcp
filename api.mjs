...
// Hook result aggregation API (wq-109)
// Shows hook success rates and trends over time
app.get("/status/hooks", (req, res) => {
  const format = req.query.format || "json";
  const limit = Math.min(parseInt(req.query.limit) || 50, 200);
  const hook = req.query.hook; // Optional: filter by hook name
  const type = req.query.type || "post"; // "pre" or "post" (default: post)
  try {
    const logDir = join(homedir(), ".config/moltbook/logs");
    const resultsFile = join(logDir, type === "pre" ? "pre-hook-results.json" : "hook-results.json");

    if (!existsSync(resultsFile)) {
      return res.json({ error: `${type}-hook-results.json not found` });
    }
    ...
    // Get last N entries (most recent first for display)
    entries = entries.slice(-limit);

    // Aggregate stats per hook
    const hookStats = {};
    let totalPass = 0, totalFail = 0, totalSessions = 0;
    for (const e of entries) {
      totalSessions++;
      totalPass += e.pass || 0;
      totalFail += e.fail || 0;
      for (const h of e.hooks || []) {
        if (hook && h.hook !== hook) continue;
        if (!hookStats[h.hook]) {
          hookStats[h.hook] = { name: h.hook, runs: 0, pass: 0, fail: 0, total_ms: 0, recent_status: [] };
        }
        const stat = hookStats[h.hook];
        stat.runs++;
        if (h.status === "ok") stat.pass++;
        else stat.fail++;
        stat.total_ms += h.ms || 0;
        stat.recent_status.push({ session: e.session, status: h.status, ms: h.ms });
        if (stat.recent_status.length > 10) stat.recent_status.shift();
      }
    }

    // Compute derived metrics
    const hookList = Object.values(hookStats).map(s => ({
      name: s.name,
      runs: s.runs,
      pass: s.pass,
      fail: s.fail,
      success_rate: s.runs > 0 ? Math.round((s.pass / s.runs) * 100) : 0,
      avg_ms: s.runs > 0 ? Math.round(s.total_ms / s.runs) : 0,
      recent: s.recent_status.slice(-5).map(r => r.status === "ok" ? "✓" : "✗").join(""),
    })).sort((a, b) => a.name.localeCompare(b.name));

    // Identify flaky hooks (success rate < 100% but > 0%)
    const flakyHooks = hookList.filter(h => h.success_rate > 0 && h.success_rate < 100);
    const failingHooks = hookList.filter(h => h.success_rate === 0);
    ...
    const result = {
      ...
      overall_success_rate: totalSessions > 0 ? Math.round((totalPass / (totalPass + totalFail)) * 100) : 0,
      unique_hooks: hookList.length,
      flaky_count: flakyHooks.length,
      failing_count: failingHooks.length,
      hooks: hookList,
      flaky: flakyHooks.map(h => h.name),
      failing: failingHooks.map(h => h.name),
    };

    if (format === "html") {
      const rows = hookList.map(h => {
        const color = h.success_rate === 100 ? "#a6e3a1" : h.success_rate === 0 ? "#f38ba8" : "#fab387";
        return `<tr>
          <td>${h.name}</td>
          <td>${h.runs}</td>
          <td style="color:${color}">${h.success_rate}%</td>
          <td>${h.avg_ms}ms</td>
          <td style="font-family:monospace">${h.recent}</td>
        </tr>`;
      }).join("");
      return res.send(`<!DOCTYPE html><html><head><title>Hook Stats</title><style>body{background:#1e1e2e;color:#cdd6f4;font-family:monospace;padding:2rem}h1{color:#89b4fa}table{border-collapse:collapse;width:100%}th,td{border:1px solid #313244;padding:0.5rem;text-align:left}th{background:#313244;color:#cba6f7}.stats{display:flex;gap:2rem;margin-bottom:1rem;flex-wrap:wrap}.stat{background:#313244;padding:0.5rem 1rem;border-radius:4px}</style></head><body>
<h1>Hook Stats (${type}-session)</h1>
<div class="stats">
  <div class="stat">Sessions: ${result.total_sessions}</div>
  <div class="stat">Overall: <span style="color:#a6e3a1">${result.overall_success_rate}%</span></div>
  <div class="stat" style="color:#fab387">Flaky: ${result.flaky_count}</div>
  <div class="stat" style="color:#f38ba8">Failing: ${result.failing_count}</div>
</div>
${flakyHooks.length ? `<p style="color:#fab387">Flaky hooks: ${result.flaky.join(", ")}</p>` : ""}
${failingHooks.length ? `<p style="color:#f38ba8">Failing hooks: ${result.failing.join(", ")}</p>` : ""}
<table>
  <tr><th>Hook</th><th>Runs</th><th>Success</th><th>Avg Time</th><th>Recent</th></tr>
  ${rows || "<tr><td colspan=5><em>No hooks</em></td></tr>"}
</table>
<p style="margin-top:2rem;color:#555;font-size:.75rem">
  Type: <a href="/status/hooks?type=post" style="color:#89b4fa">post</a> ·
  <a href="/status/hooks?type=pre" style="color:#89b4fa">pre</a> ·
  <a href="/status/hooks?format=json" style="color:#89b4fa">JSON</a> ·
  <a href="/status/inject-usage" style="color:#89b4fa">Injects</a> ·
  <a href="/status/dashboard" style="color:#89b4fa">Dashboard</a>
</p>
</body></html>`);
    }
