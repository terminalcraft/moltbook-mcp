...
# --- Prompt inject blocks (R#76, R#110: manifest-driven, wq-105: usage tracking, wq-130: conditional deps) ---
# Restructured from hardcoded INJECT_SPECS to external prompt-inject.json manifest.
# Manifest specifies: file, action (keep|consume), priority, sessions, description, requires.
...
# Conditional dependencies (wq-130): 'requires' field specifies injects that must be applied first.
...
INJECT_SKIPPED_REQUIRES=()

if [ -f "$INJECT_MANIFEST" ]; then
  # Parse manifest and process injections matching current session type
  # Two-pass approach for dependency resolution:
  # Pass 1: Identify which files exist and match session type (candidates)
  # Pass 2: Apply in priority order, checking requires against already-applied set
  declare -A INJECT_CANDIDATES  # fname -> action
  declare -A INJECT_REQUIRES    # fname -> comma-separated requires

  while IFS=$'\t' read -r fname action sessions requires; do
    [ -z "$fname" ] && continue
    # Session filter
    ...
    if [ -f "$fpath" ]; then
      INJECT_CANDIDATES["$fname"]="$action"
      [ -n "$requires" ] && INJECT_REQUIRES["$fname"]="$requires"
    else
...
try:
    ...
    for i in injs:
        ...
        if isinstance(req, list):
            req = ','.join(req)
        print(f\"{i['file']}\t{i.get('action','keep')}\t{i.get('sessions','')}\t{req}\")
except: pass
" 2>/dev/null)

  # Pass 2: Apply candidates in order (re-read to preserve priority order)
  declare -A INJECT_APPLIED_SET  # For fast lookup of applied injects
  while IFS=$'\t' read -r fname action sessions requires; do
    [ -z "$fname" ] && continue
    # Skip if not a candidate (session mismatch or file missing)
    [ -z "${INJECT_CANDIDATES[$fname]:-}" ] && continue

    # Check requires: all required injects must be in INJECT_APPLIED_SET
    if [ -n "${INJECT_REQUIRES[$fname]:-}" ]; then
      reqs="${INJECT_REQUIRES[$fname]}"
      all_met=1
      ...
      for req in "${REQ_ARRAY[@]}"; do
        ...
        [ -z "$req" ] && continue
        if [ -z "${INJECT_APPLIED_SET[$req]:-}" ]; then
          all_met=0
          break
        fi
      done
      if [ "$all_met" = 0 ]; then
        INJECT_SKIPPED_REQUIRES+=("$fname")
        continue
      fi
    fi
...
INJECT_USAGE_FILE="$LOG_DIR/inject-usage.json"
{
  ...
  echo -n "],\"skipped_requires\":["
  first=1
  for f in "${INJECT_SKIPPED_REQUIRES[@]:-}"; do
    [ -z "$f" ] && continue
    [ "$first" = 1 ] || echo -n ","
    echo -n "\"$f\""
    first=0
