...
export function buildAPromptBlock(ctx) {
  ...
  try {
    ...
    // wq-988: Surface auto-retired items so A session can mention in notes
    const ar = stats.auto_retired_items;
    if (ar && ar.count > 0 && !ar.stale) {
      const itemList = ar.retired.map(r => `${r.id}(age:${r.age})`).join(', ');
      lines.push(`Auto-retired: ${ar.count} item(s) retired this session: ${itemList}`);
    }
