...
export function buildEPromptBlock(ctx, deps = {}) {
  ...
  // wq-858: Staleness guard — skip if file is from a session >5 ago (1 full BBREA cycle)
  let prevEngageCtx = '';
  try {
    const raw = (fc.text(PATHS.eContext) || '').trim();
    if (raw) {
      const currentSession = parseInt(process.env.SESSION_NUM || '0');
      const sessionRefs = [...raw.matchAll(/\bs=(\d+)\b/g)].map(m => parseInt(m[1]));
      const maxRef = sessionRefs.length > 0 ? Math.max(...sessionRefs) : 0;
      if (currentSession > 0 && maxRef > 0 && currentSession - maxRef > 5) {
        // Stale context from a previous E session cycle — skip it
      } else {
...
${orchSection}${prevEngageCtx}${evalBlock}${emailBlock}${capBlock}`.trim();
}
