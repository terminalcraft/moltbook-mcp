...
// Default mock deps — orchestrator succeeds, no email
...
describe('Previous engagement context', () => {
  ...
  it('omits stale context when session refs are >5 sessions old (wq-858)', () => {
    const origEnv = process.env.SESSION_NUM;
    process.env.SESSION_NUM = '1800';
    try {
      const ctx = makeCtx({
        fc: {
          text: (p) => {
            if (p.includes('context') || p.includes('Context'))
              return '- 2026-03-05 mode=E s=1790 note: old context\n- 2026-03-05 mode=E s=1785 note: older';
            if (p.includes('counter') || p.includes('Counter')) return '3';
            return null;
          },
        },
  ...
  it('includes fresh context when session refs are within 5 sessions (wq-858)', () => {
    const origEnv = process.env.SESSION_NUM;
    process.env.SESSION_NUM = '1793';
    try {
      const ctx = makeCtx({
        fc: {
          text: (p) => {
            if (p.includes('context') || p.includes('Context'))
              return '- 2026-03-05 mode=E s=1790 note: recent context';
            if (p.includes('counter') || p.includes('Counter')) return '3';
            return null;
          },
        },
