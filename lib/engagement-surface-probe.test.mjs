...
describe('Convex write signals (wq-1077)', () => {
  it('detects useMutation() calls', () => {
    const js = `const addPost = useMutation(api.posts.create)`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'convex-useMutation'));
  });

  it('detects ctx.db.insert() calls', () => {
    const js = `await ctx.db.insert('messages', { body: text, author: identity })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'convex-db-insert'));
  });

  it('detects ctx.db.patch() calls', () => {
    const js = `await ctx.db.patch(messageId, { body: newText })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'convex-db-patch'));
  });

  it('detects ctx.db.replace() calls', () => {
    const js = `await ctx.db.replace(id, { ...newDoc })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'convex-db-replace'));
  });

  it('detects ctx.db.delete() calls', () => {
    const js = `await ctx.db.delete(messageId)`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'convex-db-delete'));
  });

  it('convex useMutation + db.insert triggers detection', () => {
    // convex-useMutation (4) + convex-db-insert (4) = 8 >= threshold
    const js = `const add = useMutation(api.posts.create); ctx.db.insert('posts', data)`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });
...
describe('Appwrite write signals (wq-1077)', () => {
  it('detects databases.createDocument() calls', () => {
    const js = `await databases.createDocument(dbId, collectionId, ID.unique(), { title: 'Hello' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'appwrite-createDocument'));
  });

  it('detects databases.updateDocument() calls', () => {
    const js = `await databases.updateDocument(dbId, collectionId, docId, { title: 'Updated' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'appwrite-updateDocument'));
  });

  it('detects databases.deleteDocument() calls', () => {
    const js = `await databases.deleteDocument(dbId, collectionId, docId)`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'appwrite-deleteDocument'));
  });

  it('appwrite create + update triggers detection', () => {
    // appwrite-createDocument (4) + appwrite-updateDocument (4) = 8 >= threshold
    const js = `databases.createDocument(db, col, id, data); databases.updateDocument(db, col, id, update)`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });

  it('single appwrite-deleteDocument does not trigger detection', () => {
    // appwrite-deleteDocument (3) < threshold of 6
    const js = `databases.deleteDocument(db, col, id)`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, false);
  });
});

describe('PocketBase write signals (wq-1077)', () => {
  it('detects pb.collection().create() calls', () => {
    const js = `await pb.collection('posts').create({ title: 'Hello', body: 'World' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'pocketbase-create'));
  });

  it('detects pb.collection().update() calls', () => {
    const js = `await pb.collection('posts').update(recordId, { title: 'Updated' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'pocketbase-update'));
  });

  it('detects pb.collection().delete() calls', () => {
    const js = `await pb.collection('posts').delete(recordId)`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'pocketbase-delete'));
  });

  it('pocketbase create + update triggers detection', () => {
    // pocketbase-create (4) + pocketbase-update (4) = 8 >= threshold
    const js = `pb.collection('posts').create(data); pb.collection('posts').update(id, update)`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });

  it('handles varied collection name quoting', () => {
    const js = `pb.collection("messages").create({ text: "hi" })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'pocketbase-create'));
  });
});

describe('Drizzle ORM write signals (wq-1081)', () => {
  it('detects db.insert().values() calls', () => {
    const js = `await db.insert(posts).values({ title: 'Hello', body: 'World' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'drizzle-insert'));
  });

  it('detects db.update().set() calls', () => {
    const js = `await db.update(posts).set({ title: 'Updated' }).where(eq(posts.id, 1))`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'drizzle-update'));
  });

  it('detects db.delete().where() calls', () => {
    const js = `await db.delete(posts).where(eq(posts.id, 1))`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'drizzle-delete'));
  });

  it('drizzle insert + update triggers detection', () => {
    // drizzle-insert (4) + drizzle-update (4) = 8 >= threshold
    const js = `db.insert(posts).values(data); db.update(posts).set(update).where(eq(posts.id, 1))`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });

  it('single drizzle-delete does not trigger detection', () => {
    // drizzle-delete (3) < threshold of 6
    const js = `db.delete(posts).where(eq(posts.id, 1))`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, false);
  });
});

describe('Prisma ORM write signals (wq-1081)', () => {
  it('detects prisma.model.create() calls', () => {
    const js = `await prisma.post.create({ data: { title: 'Hello' } })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'prisma-create'));
  });

  it('detects prisma.model.update() calls', () => {
    const js = `await prisma.post.update({ where: { id: 1 }, data: { title: 'Updated' } })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'prisma-update'));
  });

  it('detects prisma.model.upsert() calls', () => {
    const js = `await prisma.user.upsert({ where: { email }, create: data, update: data })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'prisma-upsert'));
  });

  it('detects prisma.model.delete() calls', () => {
    const js = `await prisma.post.delete({ where: { id: 1 } })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'prisma-delete'));
  });

  it('detects prisma.model.createMany() calls', () => {
    const js = `await prisma.post.createMany({ data: [{ title: 'A' }, { title: 'B' }] })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'prisma-createMany'));
  });

  it('prisma create + update triggers detection', () => {
    // prisma-create (4) + prisma-update (4) = 8 >= threshold
    const js = `prisma.post.create({ data }); prisma.comment.update({ where: { id }, data })`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });

  it('single prisma-delete does not trigger detection', () => {
    // prisma-delete (3) < threshold of 6
    const js = `prisma.post.delete({ where: { id: 1 } })`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, false);
  });

  it('handles various model names', () => {
    const js = `prisma.commentReply.create({ data: { text: 'hi' } })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'prisma-create'));
  });
});

describe('Kysely query builder write signals (wq-1085)', () => {
  it('detects .insertInto().values() calls', () => {
    const js = `await db.insertInto('posts').values({ title: 'Hello', body: 'World' }).execute()`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'kysely-insert'));
  });

  it('detects .updateTable().set() calls', () => {
    const js = `await db.updateTable('posts').set({ title: 'Updated' }).where('id', '=', 1).execute()`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'kysely-update'));
  });

  it('detects .deleteFrom().where() calls', () => {
    const js = `await db.deleteFrom('posts').where('id', '=', 1).execute()`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'kysely-delete'));
  });

  it('kysely insert + update triggers detection', () => {
    // kysely-insert (4) + kysely-update (4) = 8 >= threshold
    const js = `db.insertInto('posts').values(data).execute(); db.updateTable('posts').set(update).where('id', '=', 1).execute()`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });

  it('single kysely-delete does not trigger detection', () => {
    // kysely-delete (3) < threshold of 6
    const js = `db.deleteFrom('posts').where('id', '=', 1).execute()`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, false);
  });
});

describe('Knex query builder write signals (wq-1085)', () => {
  it('detects knex().insert() calls', () => {
    const js = `await knex('posts').insert({ title: 'Hello', body: 'World' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'knex-insert'));
  });

  it('detects knex().update() calls', () => {
    const js = `await knex('posts').where('id', 1).update({ title: 'Updated' })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'knex-update'));
  });

  it('detects knex().del() calls', () => {
    const js = `await knex('posts').where('id', 1).del()`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'knex-delete'));
  });

  it('knex insert + update triggers detection', () => {
    // knex-insert (4) + knex-update (4) = 8 >= threshold
    const js = `knex('posts').insert(data); knex('comments').update({ text: 'edited' })`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, true);
    assert.ok(result.totalWeight >= DETECTION_THRESHOLD);
  });

  it('single knex-delete does not trigger detection', () => {
    // knex-delete (3) < threshold of 6
    const js = `knex('posts').where('id', 1).del()`;
    const result = scanForEngagement(js);
    assert.equal(result.detected, false);
  });

  it('handles varied table name quoting', () => {
    const js = `knex("messages").insert({ text: "hi" })`;
    const result = scanForEngagement(js);
    assert.ok(result.matches.some(m => m.signal === 'knex-insert'));
  });
});
...
describe('ENGAGEMENT_SIGNALS', () => {
  it('has expected signal count', () => {
    assert.ok(ENGAGEMENT_SIGNALS.length >= 50, `Expected >=50 signals, got ${ENGAGEMENT_SIGNALS.length}`);
  });
