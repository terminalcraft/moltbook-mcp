 ...
 * Core orchestration stays in engage-orchestrator.mjs. Circuit breaker in lib/circuit-breaker.mjs (R#310).
 */
 ...
 * @param {function} [deps.analyzeEngagement] - Override analyzeEngagement().
 */
export function handleDiversity(argv, deps = {}) {
  const analyze = deps.analyzeEngagement || analyzeEngagement;
 ...
 * @param {string[]} argv
 * @param {object} [deps] - Dependency injection for testability.
 * @param {string} [deps.historyFile] - Override path to diversity-history.json.
 */
export function handleDiversityTrends(argv, deps = {}) {
  const HISTORY_FILE = deps.historyFile || join(STATE_DIR, "diversity-history.json");
  const jsonMode = argv.includes("--json");
...
/**
 * --record-outcome <platform> <success|failure>: Record engagement outcome (wq-820).
 * @param {string[]} argv
 * @param {object} [deps] - Dependency injection for testability.
 * @param {function} [deps.recordOutcome] - Override recordOutcome from circuit-breaker.
 * @param {function} [deps.exit] - Override process.exit.
 */
export function handleRecordOutcome(argv, deps = {}) {
  const record = deps.recordOutcome;
  const exit = deps.exit || process.exit;
  const idx = argv.indexOf("--record-outcome");
  const platform = argv[idx + 1];
  const outcome = argv[idx + 2];
  if (!platform || !["success", "failure"].includes(outcome)) {
    console.error("Usage: --record-outcome <platform> <success|failure>");
    exit(1);
    return { exitCode: 1 };
  }
  const result = record(platform, outcome === "success");
...
/**
 * --circuit-status: Show circuit breaker state for all platforms (wq-820).
 * @param {string[]} argv
 * @param {object} [deps] - Dependency injection for testability.
 * @param {function} [deps.loadCircuits] - Override loadCircuits from circuit-breaker.
 * @param {function} [deps.getCircuitState] - Override getCircuitState from circuit-breaker.
 * @param {function} [deps.exit] - Override process.exit.
 */
export function handleCircuitStatus(argv, deps = {}) {
  const load = deps.loadCircuits;
  const getState = deps.getCircuitState;
  const exit = deps.exit || process.exit;
  const circuits = load();
  const status = {};
  for (const [platform, entry] of Object.entries(circuits)) {
    status[platform] = { state: getState(circuits, platform), ...entry };
  }
...
/**
 * --quality-check "text": Pre-post quality gate (d066). Returns exit code (0=pass, 1=blocked).
 * @param {string[]} argv
 * @param {string} scriptDir
 * @param {object} [deps] - Dependency injection for testability.
 * @param {function} [deps.execSync] - Override child_process.execSync.
 * @param {function} [deps.exit] - Override process.exit (default: process.exit).
 */
export function handleQualityCheck(argv, scriptDir, deps = {}) {
