...
import { handleHistory, handleDiversity, handleDiversityTrends, handleQualityCheck, handleRecordOutcome, handleCircuitStatus } from './orchestrator-cli.mjs';
...
describe('handleHistory', () => {
  ...
  it('shows "ready for retry" for half-open circuits (JSON)', () => {
    const logs = [];
    const origLog = console.log;
    console.log = (...args) => logs.push(args.join(' '));
    ...
    try {
      const circuits = {
        'Recovering': {
          ...
          total_successes: 5,
          total_failures: 8,
          last_failure: new Date(Date.now() - 86400000).toISOString(),
          last_success: new Date(Date.now() - 172800000).toISOString(),
        },
      };
      const deps = makeDeps(circuits);
      deps.getCircuitState = () => 'half-open';
      handleHistory(['--json'], deps);

      const output = JSON.parse(logs.join(''));
      assert.equal(output.diagnostics[0].retryInfo, 'ready for retry');
      assert.equal(output.diagnostics[0].state, 'half-open');
    } finally {
      console.log = origLog;
    }
  });

  it('shows "ready for retry" in table format for half-open circuits', () => {
    const logs = [];
    const origLog = console.log;
    console.log = (...args) => logs.push(args.join(' '));
    ...
    try {
      ...
      const output = logs.join('\n');
      assert.match(output, /half-open/);
      assert.match(output, /ready for retry/);
    } finally {
      console.log = origLog;
    }
  });
...
// ========== handleRecordOutcome (with DI — wq-820) ==========

describe('handleRecordOutcome', () => {
  function captureLogs(fn) {
    const logs = [];
    const errors = [];
    const origLog = console.log;
  ...
  it('records success outcome and exits 0', () => {
    const exitCodes = [];
    const recorded = [];
    const deps = {
      recordOutcome: (platform, success) => {
        recorded.push({ platform, success });
        return { platform, state: 'closed', consecutive_failures: 0, total_successes: 1 };
      },
      exit: (code) => exitCodes.push(code),
    };
    const { logs, result } = captureLogs(() =>
      handleRecordOutcome(['--record-outcome', 'Moltbook', 'success'], deps)
    );
    assert.equal(result.exitCode, 0);
    assert.deepEqual(exitCodes, [0]);
    assert.deepEqual(recorded, [{ platform: 'Moltbook', success: true }]);
    const output = JSON.parse(logs.join(''));
    assert.equal(output.platform, 'Moltbook');
    assert.equal(output.state, 'closed');
  });

  it('records failure outcome and exits 0', () => {
    const exitCodes = [];
    const recorded = [];
    const deps = {
      recordOutcome: (platform, success) => {
        recorded.push({ platform, success });
        return { platform, state: 'open', consecutive_failures: 3, total_failures: 3 };
      },
      exit: (code) => exitCodes.push(code),
    };
    const { result } = captureLogs(() =>
      handleRecordOutcome(['--record-outcome', 'BadPlatform', 'failure'], deps)
    );
    assert.equal(result.exitCode, 0);
    assert.deepEqual(exitCodes, [0]);
    assert.deepEqual(recorded, [{ platform: 'BadPlatform', success: false }]);
  });

  it('exits 1 when platform is missing', () => {
    const exitCodes = [];
    const deps = {
      recordOutcome: () => { throw new Error('should not be called'); },
      exit: (code) => exitCodes.push(code),
    };
    const { errors, result } = captureLogs(() =>
      handleRecordOutcome(['--record-outcome'], deps)
    );
  ...
  it('exits 1 when outcome is invalid', () => {
    const exitCodes = [];
    const deps = {
      recordOutcome: () => { throw new Error('should not be called'); },
      exit: (code) => exitCodes.push(code),
    ...
    const { errors, result } = captureLogs(() =>
      handleRecordOutcome(['--record-outcome', 'Platform', 'maybe'], deps)
    );
    assert.equal(result.exitCode, 1);
    assert.deepEqual(exitCodes, [1]);
    assert.ok(errors.join('\n').includes('Usage:'));
  ...
  it('works with --record-outcome not at argv start', () => {
    const exitCodes = [];
    const deps = {
      recordOutcome: (platform, success) => ({ platform, success }),
      exit: (code) => exitCodes.push(code),
    };
    const { result } = captureLogs(() =>
      handleRecordOutcome(['node', 'script.mjs', '--record-outcome', 'Chatr', 'success'], deps)
    );
    assert.equal(result.exitCode, 0);
    assert.equal(result.result.platform, 'Chatr');
  });
});

// ========== handleCircuitStatus (with DI — wq-820) ==========

describe('handleCircuitStatus', () => {
  function captureLogs(fn) {
    const logs = [];
    const origLog = console.log;
    console.log = (...args) => logs.push(args.join(' '));
    let result;
    try { result = fn(); } finally { console.log = origLog; }
    return { logs, result };
  }

  it('outputs circuit status as JSON and exits 0', () => {
    const exitCodes = [];
    const deps = {
      loadCircuits: () => ({
        Moltbook: { consecutive_failures: 0, total_successes: 10, total_failures: 2 },
        Chatr: { consecutive_failures: 3, total_successes: 5, total_failures: 8 },
      }),
      getCircuitState: (circuits, platform) => {
        return (circuits[platform]?.consecutive_failures || 0) >= 3 ? 'open' : 'closed';
      },
      exit: (code) => exitCodes.push(code),
    };
    const { logs, result } = captureLogs(() =>
      handleCircuitStatus([], deps)
    );
    assert.equal(result.exitCode, 0);
    assert.deepEqual(exitCodes, [0]);
    const output = JSON.parse(logs.join(''));
    assert.equal(output.Moltbook.state, 'closed');
    assert.equal(output.Moltbook.total_successes, 10);
    assert.equal(output.Chatr.state, 'open');
    assert.equal(output.Chatr.consecutive_failures, 3);
  });
  ...
  it('merges state into circuit entry correctly', () => {
    const exitCodes = [];
    const deps = {
      loadCircuits: () => ({
        TestPlatform: {
          ...
          total_failures: 10,
          total_successes: 3,
          last_failure: '2026-03-01T00:00:00Z',
        },
      }),
      getCircuitState: () => 'half-open',
      exit: (code) => exitCodes.push(code),
    };
    const { logs } = captureLogs(() =>
      handleCircuitStatus([], deps)
    ...
    const output = JSON.parse(logs.join(''));
    assert.equal(output.TestPlatform.state, 'half-open');
    assert.equal(output.TestPlatform.consecutive_failures, 5);
    assert.equal(output.TestPlatform.last_failure, '2026-03-01T00:00:00Z');
  });

  it('returns status in result object', () => {
    const deps = {
      loadCircuits: () => ({ A: { consecutive_failures: 0 } }),
      getCircuitState: () => 'closed',
      exit: () => {},
    };
    const { result } = captureLogs(() =>
    ...
    assert.equal(result.status.A.state, 'closed');
  });
});
