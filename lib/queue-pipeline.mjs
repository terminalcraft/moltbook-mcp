...
export function runQueuePipeline({ MODE, COUNTER, fc, PATHS, DIR, result, readJSON, markTiming }) {
  ...
  // R#361: Directive-priority enforcement — items tagged with an active directive
  // sort before all others. Previously this was textual-only in SESSION_BUILD.md
  // and was skipped by 2 consecutive B sessions (s2003, s2004), stalling d078.
  const BUDGET_CAP = parseFloat(process.env.BUDGET_CAP || '10');
  if (MODE === 'B' && pending.length > 0) {
    // Load active directive IDs for priority sorting
    let activeDirectiveIds = [];
    try {
      const dData = fc.json(PATHS.directives);
      activeDirectiveIds = (dData.directives || [])
        .filter(d => d.status === 'active' || d.status === 'in-progress')
        .map(d => d.id);
    } catch { /* directives unavailable — skip priority boost */ }

    const prioritized = [...pending].sort((a, b) => {
      const aTags = a.tags || [];
      const bTags = b.tags || [];
      // Directive-tagged items first
      const aDir = aTags.some(t => activeDirectiveIds.includes(t)) ? 0 : 1;
      const bDir = bTags.some(t => activeDirectiveIds.includes(t)) ? 0 : 1;
      if (aDir !== bDir) return aDir - bDir;
      // Then audit-tagged items
      const aAudit = aTags.includes('audit') ? 0 : 1;
      const bAudit = bTags.includes('audit') ? 0 : 1;
      return aAudit - bAudit;
    });
    const sized = prioritized.map(i => ({ ...i, _c: (i.complexity || 'M').toUpperCase() }));
    const preferred = sized.filter(i => i._c !== 'L');
    const item = (preferred.length > 0 && BUDGET_CAP <= 5) ? preferred[0] : prioritized[0];
  ...
  // --- Auto-promote brainstorming ideas to queue when pending < 4 ---
  // d073/R#320: Runs for ALL session types (was B/R only). E and A sessions
  // run 2x per BBREA cycle but never contributed to queue replenishment,
  // causing starvation between R sessions. Threshold raised 3→4 per d073.
  {
  ...
  if (MODE === 'B') {
    ...
    if (existsSync(todoPath)) {
      ...
      // Staleness guard (wq-852): if file is from a session >2 ago, delete without processing
      const sessionMatch = todoContent.match(/session\s+(\d+)/i);
      const fileSession = sessionMatch ? parseInt(sessionMatch[1]) : 0;
      const currentSession = parseInt(process.env.SESSION_NUM || '0');
      if (fileSession > 0 && currentSession > 0 && currentSession - fileSession > 2) {
        try { unlinkSync(todoPath); } catch { /* ignore */ }
        result.todo_stale_deleted = fileSession;
      } else {
      ...
      if (todoLines.length > 0) {
        ...
        // Also load patterns from todo-false-positives.json (shared with 27-todo-scan.sh)
        const fpPath = join(DIR, 'todo-false-positives.json');
        if (existsSync(fpPath)) {
          try {
            const fp = JSON.parse(readFileSync(fpPath, 'utf8'));
            for (const p of fp.patterns || []) {
              FALSE_POSITIVE_PATTERNS.push(new RegExp(p, 'i'));
            }
          } catch { /* ignore parse errors */ }
        }
        const isFalsePositive = (line) => FALSE_POSITIVE_PATTERNS.some(p => p.test(line));
        // wq-882: Specificity gate — reject generic TODOs that lack actionable context.
        // Must contain a file reference (.mjs/.sh/.json), function-like identifier (camelCase/snake_case),
        // or a specific scope indicator to be ingested.
        const isSpecificEnough = (line) => {
          const stripped = line.replace(/^\/\/\s*/, '').replace(/^#\s*/, '').replace(/\b(TODO|FIXME|HACK|XXX)\b:?\s*/i, '');
          // Has a file reference
          if (/\w+\.\w{1,4}\b/.test(stripped) && /\.(mjs|js|sh|json|py|ts|md)\b/.test(stripped)) return true;
          // Has a function/variable name (camelCase or snake_case with 2+ segments)
          if (/[a-z][a-zA-Z]*[A-Z][a-zA-Z]*/.test(stripped)) return true;
          if (/[a-z]+_[a-z]+/.test(stripped)) return true;
          // Has a URL path or endpoint reference
          if (/\/\w{2,}/.test(stripped)) return true;
          // Has a specific identifier like wq-NNN, s1234, etc.
          if (/\bwq-\d{3,}/.test(stripped)) return true;
          if (/\bs\d{4}\b/.test(stripped)) return true;
          // Remaining text has enough unique words (>=4 non-stop words) to be specific
          const words = stripped.toLowerCase().split(/\W+/).filter(w => w.length > 3 && !STOP_WORDS.has(w));
        ...
        for (let i = 0; i < todoLines.length && i < 3; i++) {
          ...
          if (!isSpecificEnough(raw)) continue;
      ...
      // Delete followups file after processing to prevent re-ingestion
      try { unlinkSync(todoPath); } catch { /* ignore */ }
      } // end staleness guard else
    }
  }
