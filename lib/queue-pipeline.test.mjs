...
describe('runQueuePipeline', () => {
  ...
  describe('auto-promote', () => {
    it('promotes brainstorming ideas when pending < 4 (d073/R#320)', () => {
      writeFileSync(join(SCRATCH, 'work-queue.json'), JSON.stringify({
        queue: [
    ...
    it('does not promote when pending >= 4 (d073/R#320)', () => {
      writeFileSync(join(SCRATCH, 'work-queue.json'), JSON.stringify({
        queue: [
          ...
          { id: 'wq-004', title: 'Task four', status: 'pending', priority: 4 },
        ]
      }));
      writeFileSync(join(SCRATCH, 'BRAINSTORMING.md'),
    ...
    it('promotes in all session modes including E (d073/R#320)', () => {
      writeFileSync(join(SCRATCH, 'work-queue.json'), JSON.stringify({ queue: [] }));
      writeFileSync(join(SCRATCH, 'BRAINSTORMING.md'),
        '# Ideas\n- **Great feature**: description\n- **Another feature**: desc2\n- **Third feature**: desc3\n- **Fourth feature**: desc4\n- **Fifth feature**: desc5\n');
      const opts = makePipelineOpts({ MODE: 'E' });
      runQueuePipeline(opts);
      assert.ok(opts._result.auto_promoted, 'E mode should auto-promote when queue is low');
      assert.ok(opts._result.auto_promoted.length > 0);
    });
  });

  describe('TODO ingest', () => {
    it('filters false positives from todo-false-positives.json', () => {
      writeFileSync(join(SCRATCH, 'work-queue.json'), JSON.stringify({
        queue: [{ id: 'wq-001', title: 'Existing task', status: 'pending', priority: 1 }]
      }));
      // Write a followups file with a false positive and a real TODO
      writeFileSync(join(SCRATCH, 'todo-followups.txt'),
        '## Follow-up items\n- \'xxx\', \'TODO\', \'REPLACE_ME\'\n- TODO: real issue to fix in parser\n');
      // Write false-positives config matching the first line
      writeFileSync(join(SCRATCH, 'todo-false-positives.json'), JSON.stringify({
        patterns: ["'xxx',\\s*'TODO',\\s*'REPLACE_ME'"]
      }));
      const opts = makePipelineOpts({ MODE: 'B' });
      const { queue } = runQueuePipeline(opts);
      // Only the real TODO should be ingested, not the false positive
      const todoItems = queue.filter(q => q.title && q.title.includes('TODO followup'));
      assert.equal(todoItems.length, 1);
      assert.ok(todoItems[0].title.includes('real issue'));
    });

    it('deletes stale followups file without processing (wq-852)', () => {
      writeFileSync(join(SCRATCH, 'work-queue.json'), JSON.stringify({
        queue: [{ id: 'wq-001', title: 'Task', status: 'pending', priority: 1 }]
      }));
      const followupsPath = join(SCRATCH, 'todo-followups.txt');
      // File from session 100, current session is 105 (>2 sessions old)
      writeFileSync(followupsPath, '## Follow-up items from session 100\n- TODO: stale item should not be ingested\n');
      const origEnv = process.env.SESSION_NUM;
      process.env.SESSION_NUM = '105';
      try {
        const opts = makePipelineOpts({ MODE: 'B' });
        const { queue } = runQueuePipeline(opts);
        assert.equal(existsSync(followupsPath), false, 'stale followups file should be deleted');
        const todoItems = queue.filter(q => q.title && q.title.includes('TODO followup'));
        assert.equal(todoItems.length, 0, 'stale TODOs should not be ingested');
      } finally {
        if (origEnv !== undefined) process.env.SESSION_NUM = origEnv;
        else delete process.env.SESSION_NUM;
      }
    });

    it('deletes followups file after processing', () => {
      writeFileSync(join(SCRATCH, 'work-queue.json'), JSON.stringify({
        queue: [{ id: 'wq-001', title: 'Task', status: 'pending', priority: 1 }]
      }));
      const followupsPath = join(SCRATCH, 'todo-followups.txt');
      writeFileSync(followupsPath, '## Follow-ups\n- TODO: check this thing\n');
      const opts = makePipelineOpts({ MODE: 'B' });
      runQueuePipeline(opts);
      assert.equal(existsSync(followupsPath), false, 'followups file should be deleted after processing');
    });
  });
