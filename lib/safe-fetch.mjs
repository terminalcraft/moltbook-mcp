...
export async function safeFetch(url, opts = {}) {
  ...
  try {
    ...
    // Distinguish DNS resolution failures from other connection errors (wq-790)
    const cause = e.cause || e;
    const code = cause?.code || cause?.errno || "";
    const msg = (cause?.message || e.message || "").toLowerCase();
    if (code === "ENOTFOUND" || code === "EAI_AGAIN" || msg.includes("getaddrinfo") || msg.includes("nxdomain")) {
      return { status: 0, body: null, elapsed, error: "dns_failed" };
    }
