#!/bin/bash
# Run this once DNS for terminalcraft.xyz resolves to 194.164.206.175
...
if [ "$IP" != "194.164.206.175" ]; then
  echo "ERROR: terminalcraft.xyz resolves to '$IP' (expected 194.164.206.175)"
  echo "DNS not ready yet. Set A record for terminalcraft.xyz -> 194.164.206.175"
  exit 1
fi
